...
struct Libro {
    ...
    std::string generos;
    std::string imagen;
    std::string fechaDevolucion; // NUEVO CAMPO
};
...
std::vector<Libro> cargarDesdeArchivo(const std::string& ruta) {
    ...
    if (archivo.is_open()) {
        ...
        while (std::getline(archivo, linea)) {
            ...
            std::string titulo, autor, persona, prestadoStr, generos, imagen, fechaDevolucion;
            ...
            if (std::getline(ss, titulo, '|') &&
                ...
                biblioteca.push_back({titulo, autor, persona, prestadoStr == "1", generos, imagen, fechaDevolucion});
            }
        }
        archivo.close();
    }
...
Java_com_example_libros2_RegistrarActivity_registrarLibro(
        ...
        jstring persona, jboolean prestado, jstring generos, jstring imagen, jstring fecha_devolucion) {
    ...
    biblioteca.push_back({cTitulo, cAutor, cPersona, prestado == JNI_TRUE, cGeneros, cImagen, cFecha});
...
Java_com_example_libros2_VerActivity_filtrarLibrosPorGenero(JNIEnv* env, jobject thiz, jstring ruta, jstring genero_filtro) {
    ...
    for (const auto& l : biblioteca) {
        if (filtro == "Todos" || l.generos.find(filtro) != std::string::npos) {
            ...
            resultado += l.titulo + "|" + l.autor + "|" + l.generos + "|" + estado + "|" + l.imagen + "|" + l.fechaDevolucion + ";";
            contador++;
        }
    }
    ...
    for (auto& l : biblioteca) {
        if (l.titulo == objetivo) {
            ...
            if (!l.prestado) {
                ...
                l.prestado = false;
                l.persona = "Ninguno";
                l.fechaDevolucion = ""; // Limpia la fecha
            }
            break;
        }
    }
...
// 6. PRESTAR LIBRO (Actualizado con la fecha)
extern "C" JNIEXPORT jstring JNICALL
Java_com_example_libros2_PrestarActivity_prestarLibro(
        JNIEnv* env, jobject thiz, jstring ruta, jstring titulo, jstring persona, jstring fecha_devolucion) {
